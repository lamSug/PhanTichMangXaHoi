...
def get_status(x):
    status = ""
    try:
        status = x.find_element(By.XPATH,  ".//div[@class='_5wj-']").text
    except:
        try:
            status = x.find_element(By.XPATH,  ".//div[@class='userContent']").text
        except:
            pass
    return status
...
def get_div_links(x, tag):
    try:
        temp = x.find_element(By.XPATH,  ".//div[@class='_3x-2']")
...
def get_title(x):
    title = ""
    try:
        title = x.find_element(By.XPATH,  ".//span[@class='fwb fcg']")
    except:
        try:
            title = x.find_element(By.XPATH,  ".//span[@class='fcg']")
        except:
            try:
                title = x.find_element(By.XPATH,  ".//span[@class='fwn fcg']")
            except:
                pass
    finally:
        return title
        ...
        for x in elements:
            try:
                ...
                if title.text.find("shared a memory") != -1:
                    x = x.find_element(By.XPATH,  ".//div[@class='_1dwg _1w_m']")
                    title = get_title(x)

                status = get_status(x)
                if title.text == driver.find_element(By.ID, "fb-timeline-cover-name").text:
                    if status == '':
    ...
    try:
        ...
        if status == 0:
            ...
            for i in range(len(results)):
                f.writelines("\n"+ results[i])
                result_id = results[i].split('/')[-1]
                if result_id not in input_data:
                    f_input.write(result_id + '\n')

            # get names of friends
            # for x in elements:
            #     print(x)
            # people_names = [x.find_element(By.TAG_NAME, "img").get_attribute("aria-label") for x in elements]
            # for i in range(len(results)):
        ...
        #                 img_links = [x.find_element(By.CSS_SELECTOR, 'img').get_attribute('src') for x in elements]
        #             else:
        #                 links = []
        ...
        #                         l = driver.find_element(By.CLASS_NAME, "profilePicThumb").get_attribute('href')
        #                     except:
        #                         l = "None"
        #
        #                     links.append(l)
        ...
        #                 background_img_links = driver.find_elements(By.XPATH, "//*[contains(@id, 'pic_')]/div/i")
        ...
        # # dealing with Videos
        # elif status == 2:
        #     results = elements[0].find_elements(By.CSS_SELECTOR, 'li')
        #     results = [x.find_element(By.CSS_SELECTOR, 'a').get_attribute('href') for x in results]
        #
        #     try:
        #         if results[0][0] == '/':
    ...
    for i in range(len(scan_list)):
        try:
            ...
            if (save_status == 0) or (save_status == 1) or (
                ...
                # sections_bar = driver.find_element(By.XPATH,  "//*[@class='xyamay9 x1pi30zi x1l90r2v x1swvt13'][1]/div[2]/div[1]")

                # if sections_bar.text.find(scan_list[i]) == -1:
                #     continue
            d = driver.find_elements(By.XPATH, './/div[@class=\'x78zum5 x1q0g3np x1a02dak x1qughib\'][1]/div/div/a')
            print(len(d))
            ...
            data = driver.find_elements(By.XPATH, './/div[@class=\'x78zum5 x1q0g3np x1a02dak x1qughib\'][1]/div/div/a')
...
def main():
    ...
    f_crawled = open("crawled.txt", 'a+')
    f_crawled.seek(0)
    crawled_node = f_crawled.readlines()
    crawled_node = [cnode.split('\n')[0] for cnode in crawled_node]
    f_crawled.close()
    ids = []
    f = open("input.txt", newline='\n')
    for line in f:
        if line=='\r\n':
            continue
        line_fomarted = line.split('\r')[0]
        if line == '100044906408033\r\n':
            print(1)
        if line_fomarted not in crawled_node:
            ids.append("https://facebook.com/" + line_fomarted)
    # ids = ["https://en-gb.facebook.com/" + line for line in f if line not in crawled_node]
    f.close()
    if len(ids) > 0:
        print("\nStarting Scraping...")
